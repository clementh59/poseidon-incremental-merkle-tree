...
const abi = require('../artifacts/contracts/IncrementalMerkleTree.sol/IncrementalMerkleTree.json').abi;
...
const interactWithMerkleTree = async () => {
    ...
    let status = true;
    if (status)
        status = await addLeaf(contract, '0x1399C190AEEDDC5D8B2242AB17F97B42631BB0B7877E92B92C3BE2EC5685A6A1');
    if (status)
        status = await addLeaf(contract, '0x928369EB910E972BE21CE00310D5D3544718BB02979F49D13B704759F2D888E');
    if (status)
        status = await addLeaf(contract, '0x86EAD858855392BB66E138A5BB841E85AA2BAE6CC4FEACD41EA1C63C7AE173F');
    
    console.log(`Every leaves were added correctly? ${status}`);
